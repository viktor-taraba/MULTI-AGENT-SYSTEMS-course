...
async def run_critic(user_text: str) -> str:
    async with Client(port_search) as mcp_client:
        ...
        try:
            ...
            async for step in critic_agent.astream({"messages": messages_history}):
            ...
            if type(e).__name__ == "ToolCallLimitExceededError":
                print_agent_step(f"\n⚠️ Researcher reached tool call limit. Forcing final output...",agent_name="Critic")
                ...
                messages_history.append(("user", FINAL_PROMPT_critic))
                fallback_agent = create_agent(
                    model=critic_model_name,
                    tools=[], 
                    system_prompt=FINAL_PROMPT_critic,
                    response_format=CritiqueResult,
                    middleware=[] 
                )
