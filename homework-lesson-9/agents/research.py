...
async def run_research(user_text: str) -> str:
    async with Client(port_search) as mcp_client:
        ...
        try:
            ...
            async for step in research_agent.astream({"messages": messages_history}):
            ...
            if type(e).__name__ == "ToolCallLimitExceededError":
                print_agent_step(f"\n⚠️ Researcher reached tool call limit. Forcing final output...",agent_name="Researcher")
                ...
                messages_history.append(("user", FINAL_PROMPT_research))
                fallback_agent = create_agent(
                    model=research_model_name,
                    tools=[], 
                    system_prompt=FINAL_PROMPT_research,
                    response_format=ResearchResult,
                    middleware=[] 
                )
