...
from helper import evaluate_and_assert, unique_tool_names
...
def test_planner_tools():
    user_input = "Create a rsearch plan for agentic development for MS Power BI"
    agent_response = planner_agent.invoke(
            {"messages": [("user", user_input)]}, 
            config={"configurable": {"thread_id": "test_thread_001"}}
        )

    unique_tool_names = unique_tool_names(agent_response)
